...
%for nClassifiers = 1:maxNumberClassifiers
    ...
    % Each loop iteration trains one weak classifier and then updates the
    % weights D.
    for classifier = 1:maxNumberClassifiers
    ...
    for weakClass = 1:classifier
        
        haar_idx = weak_classifiers(weakClass, 1);
        threshold = weak_classifiers(weakClass, 2);
        polarity = weak_classifiers(weakClass, 3);
        error = weak_classifiers(weakClass, 4);
    ...
    numberOfClassifiers = [numberOfClassifiers, classifier];
    % Also save the configurations for the strong classifier.
    strong_classifiers(:,:, classifier) = weak_classifiers;
    
    % Display some info on the training progress
    clc;
    time_elapsed = time_elapsed + toc;
    if estimated_time < 0.1
        estimated_time = time_elapsed * maxNumberClassifiers;
    end
    disp(['Training weak classifier ',num2str(classifier), ' of ' num2str(maxNumberClassifiers), '. ']);
    disp([num2str((classifier / maxNumberClassifiers) * 100), '% Done.']);
